...
// ── Nodemailer — HostAfrica cPanel SMTP ───────────────────────────────────────
const mailer = nodemailer.createTransport({
    host: process.env.EMAIL_HOST,      // e.g. mail.yourdomain.com
    port: 587,                         // SMTP port (STARTTLS)
    secure: false,                     // false = STARTTLS on port 587 | set true for port 465
    auth: {
        user: process.env.EMAIL_USER,    // full email address e.g. noreply@yourdomain.com
        pass: process.env.EMAIL_PASS,    // your email account password
    },
    tls: {
        rejectUnauthorized: false,       // needed for cPanel shared hosting certificates
    },
    logger: true,
    debug: true,
});
